...
--EXEC [dbo].[USP_API_FeedMonitor_UserEvent_Stats] 634
DECLARE 
@UserID INT = 634
...
select  ClientID,e.EventID,e.CreatedAt into #Data from EventUser eu with (nolock)
...
BEGIN
		...
		DECLARE @SelectQuery NVARCHAR(2000)='',@WhereQuery NVARCHAR(2000)='',@StatsQuery NVARCHAR(2000)='',@DetailsQuery NVARCHAR(2000)=''
...
select count(distinct b.ClientID) as total_clients,count(distinct b.EventID) as total_events,count(Clients_Added_In_Last_30Days.clientid) as Clients_Added_In_Last_30Days,Count(EventID) as Events_Added_In_Last_30Days,
...
OUTER APPLY (select distinct c.clientid from Client c WITH(NOLOCK) where b.ClientID=c.ClientID and CreatedAt>=convert(date,GETDATE()-30)) as Clients_Added_In_Last_30Days
OUTER APPLY (select distinct EventID from #Data where CreatedAt>=convert(date,GETDATE()-30)) as Events_Added_In_Last_30Days



;with CTE As(
select 
DISTINCT CASE  WHEN (ISNULL(Health, '') = 'Negative' OR ISNULL(Healthinserted, '') = 'Negative' OR ISNULL(HealthRelevant, '') = 'Negative' OR Health IS NULL )THEN b.EventID 
WHEN  (ISNULL(Health, '') = 'Negative' and Healthinserted IS NULL and HealthRelevant IS NULL ) THEN b.EventID 
WHEN Health IS NULL THEN b.EventID 
    END AS negative_events
from #Events_HealthData a WITH(NOLOCK)
...
select * from CTE where negative_events is not null;

;with CTE1 As(
SELECT
 CASE  WHEN (ISNULL(Health, '') = 'Positive' and ( ISNULL(Healthinserted, '') = 'Positive' AND ISNULL(HealthRelevant, '') = 'Positive')) THEN b.EventID 
WHEN (ISNULL(Health, '') = 'Positive' and Healthinserted IS NULL and HealthRelevant IS NULL ) THEN b.EventID 
    END AS positive_events
from #Events_HealthData a WITH(NOLOCK)
RIGHT JOIN #Data b WITH(NOLOCK) ON a.EventID=b.EventID 
)
select * from CTE1 where positive_events is not null;
...
---- Store relevant TagIDs for the Event
--DECLARE @TagIDs TABLE (TagID INT);
--INSERT INTO @TagIDs (TagID)
--SELECT TagID 
--FROM Event a WITH (NOLOCK) 
--INNER JOIN #Data b WITH (NOLOCK) ON a.EventID = b.EventID 
----WHERE a.EventID = 160;

---- Get the earliest records per URL
--;WITH CTE1 AS (
--    SELECT 
...
--    WHERE TopicID IN (SELECT TagID FROM @TagIDs)
--)
--SELECT 
--    Date, 
--    url, 
...
---- Get the latest records per URL
--;WITH CTE2 AS (
--    SELECT 
--        ROW_NUMBER() OVER (PARTITION BY url ORDER BY createdat DESC) AS rn,
--        url,
--        comment,
--        Date,
--        TopicID
...
---- Combine the earliest and latest records
--SELECT 
--    T1.SerpTab,
--    T1.Url,
--    '' AS Headline,
--    T2.comment AS LatestComment,
--    T1.Date AS ArticleDate,
--    0 AS IsEnriched,
--    0 AS IsRelevant,  
--    T2.TopicID AS TagID
--INTO #Temp_ArticleOnlineLog
--FROM #TEMP1 T1
--INNER JOIN #TEMP2 T2 ON T1.Url = T2.Url;

---- Get the counts grouped by TagID
--SELECT 
--    COUNT(*) AS [Total Records], 
--    SUM(CASE WHEN LatestComment = 'Inserted In DB' THEN 1 ELSE 0 END) AS [Inserted In DB],
--    TagID
--FROM #Temp_ArticleOnlineLog
--GROUP BY TagID;

---- Clean up temporary tables
--DROP TABLE #TEMP1, #TEMP2, #Temp_ArticleOnlineLog;
--drop table #Data