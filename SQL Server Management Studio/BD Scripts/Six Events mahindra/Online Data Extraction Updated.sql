...
END AS [Competitor_spokesperson_in_headline_Kia_India]
			...
			WHERE e.EventID=@EventID AND LP.Date >= '2025-04-01 00:00:00' AND LP.Date <= '2025-07-21 23:00:00' 
			--AND (
...
select  ROW_NUMBER() over(partition by TLOMEID order by TLOMEID  ) as RN,*  from [dbo].Tag_MGMotor_21072025_Online where TLOMEID in (select TLOMEID from CTE_TEMPDATA)
			)
