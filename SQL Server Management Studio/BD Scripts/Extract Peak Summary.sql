...
select a.eventID,Count(1)  from [dbo].[Event_LinkPrint_Summary] a with(nolock)
INNER JOIN Event b with(nolock) on a.eventid=b.eventid
where a.EventID in (154,155,156,157,158,159)
and createdat >='2025-04-18'
group by a.eventID
...
select * from Event_LinkOnline_Summary where EventID=154 and peakid >0 order by PeakID desc
 
select * from Event_LinkPrint_Summary where EventID=154 and peakid >0 order by PeakID desc
