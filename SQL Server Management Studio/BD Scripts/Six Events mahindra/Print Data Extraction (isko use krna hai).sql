...
select TLPMEID 
		...
		and NewsDate >= '2025-07-01 00:00:00' 
		and NewsDate <= '2025-08-31 23:59:00'  
...
END AS [Competitor_spokesperson_in_headline_Kia_India]
			...
			WHERE e.EventID=@EventID AND AO.NewsDate >= '2025-07-01 00:00:00' AND AO.NewsDate <= '2025-08-31 23:59:00'
			AND (
...
select  ROW_NUMBER() over(partition by article_id order by article_id  ) as RN,*  from [dbo].LinkPrintTagged_MGMotor_31aug where article_id in (select article_id from CTE_TEMPDATA)
			)
