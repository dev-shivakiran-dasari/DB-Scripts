...
inner join [User] b with(Nolock) on a.UserID=b.UserID
inner join Event with(Nolock) on a.EventID=Event.EventID
inner join SentryUser with(Nolock) on a.UserID=SentryUser.UserID
OUTER APPLY (
select COUNT(1) as AIH_Count from Event_Notification_Log_Detail ENLD with(Nolock)
INNER JOIN Event_Notification_Log ENL with(Nolock) on ENLD.NLID=ENL.NLID and ENLD.UserID =a.UserID and Platform_AsItHappens=1 and ENLD.MediaTypeID=a.PlatformID 
and ENL.TopicID=a.EventID and ENLD.[Status]='Success' and ENL.CreatedAt >='2025-07-17 00:00:00'
) as AIH
OUTER APPLY (
select COUNT(1) as CON_Count from Event_Notification_Log_Conditional_Detail ENLD_con with(Nolock)
INNER JOIN Event_Notification_Log_Conditional ENL_con with(Nolock) on ENLD_con.NLID=ENL_con.NLID and ENLD_con.UserID =a.UserID and Platform_Conditional=1 and ENLD_con.MediaTypeID=a.PlatformID
and ENL_con.TopicID=a.EventID and ENLD_con.[Status]='Success'  and ENL_con.CreatedAt >='2025-07-17 00:00:00'
) as Conditional
OUTER APPLY (
select COUNT(1) as SUM_Count from Event_Notification_Log_SI_Detail ENLD_Sum with(Nolock)
INNER JOIN Event_Notification_Log_SI ENL_Sum with(Nolock) on ENLD_Sum.NLID=ENL_Sum.NLID and ENLD_Sum.UserID =a.UserID and Platform_Summarised=1 and ENLD_Sum.MediaTypeID=a.PlatformID  
...
OUTER APPLY (
select COUNT(1) as AIH_Generated_Count from Event_Notification_Log_Detail ENLD with(Nolock)
INNER JOIN Event_Notification_Log ENL with(Nolock) on ENLD.NLID=ENL.NLID and ENLD.UserID =a.UserID and Platform_AsItHappens=1 and ENLD.MediaTypeID=a.PlatformID 
and ENL.TopicID=a.EventID  and ENL.CreatedAt >='2025-07-17 00:00:00'
) as AIH_Generated_Count
OUTER APPLY (
select COUNT(1) as CON_Generated_Count from Event_Notification_Log_Conditional_Detail ENLD_con with(Nolock)
INNER JOIN Event_Notification_Log_Conditional ENL_con with(Nolock) on ENLD_con.NLID=ENL_con.NLID and ENLD_con.UserID =a.UserID and Platform_Conditional=1 and ENLD_con.MediaTypeID=a.PlatformID
and ENL_con.TopicID=a.EventID  and ENL_con.CreatedAt >='2025-07-17 00:00:00'
) as Conditional_Generated_Count
OUTER APPLY (
select COUNT(1) as SUM_Generated_Count from Event_Notification_Log_SI_Detail ENLD_Sum with(Nolock)
INNER JOIN Event_Notification_Log_SI ENL_Sum with(Nolock) on ENLD_Sum.NLID=ENL_Sum.NLID and ENLD_Sum.UserID =a.UserID and Platform_Summarised=1 and ENLD_Sum.MediaTypeID=a.PlatformID  
