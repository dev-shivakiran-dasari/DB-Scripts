...
--ismai srif LinkOnlineTagged wale tables change krna hai and eventid change krna hai

declare @EventID INT =2345
...
--END AS [Competitor_spokesperson_in_headline_Kia_India]
			...
			INNER JOIN [dbo].[LinkOnline_Processed] LP WITH (NOLOCK) ON LP.LOPID=AO.LOPID and LP.Date >= '2026-02-01 00:00:00' 
			...
			INNER JOIN LinkOnlineTagged_Mahindra_25Mar2026 LOT WITH (NOLOCK) ON LOT.article_id=tlom.TLOMID
			INNER JOIN [dbo].[LinkOnline_EnrichedDetail] LED with (nolock) ON LED.TLOMEID = TLOME.TLOMEID
			INNER JOIN [dbo].[LinkOnline_Enriched] LE with (nolock) ON LE.LOID = AO.LOID and LE.LOEID = LED.LOEID			
			INNER JOIN [dbo].[EventTagGroup] etg WITH (NOLOCK) ON etg.TagID=TLOME.TagID 
			INNER JOIN [dbo].[Event] e WITH (NOLOCK) ON e.EventID=etg.EventID
			...
			WHERE e.EventID=@EventID AND LP.Date >= '2026-02-01 00:00:00' AND LP.Date <= '2026-02-28 23:59:00' 
